...
import { saveMessage, isNewConversation, updateConversationClientData, getMessages, hasEscalation } from '../storage/messageStorage.js';
import { getRandomClientData } from '../utils/clientData.js';
import { sendAgentAssistData } from '../services/websocketServer.js';

const BRIDGE_URL = process.env.BRIDGE_URL || 'https://whatsapp-integration-6ofl.onrender.com';
const DEFAULT_PHONE_NUMBER = process.env.DEFAULT_PHONE_NUMBER || '5511983461478';
 ...
 */
const sendMessageToBridge = async (text, phoneNumber = DEFAULT_PHONE_NUMBER) => {
  if (!BRIDGE_URL) {
...
const processBuffers = (buffers, mimetypes, filenames, type, uploadDir) => {
  ...
  for (let i = 0; i < buffers.length; i++) {
    ...
    if (Buffer.isBuffer(buffers[i])) {
      ...
      buffer = Buffer.from(buffers[i].buffer, buffers[i].byteOffset, buffers[i].byteLength);
      ...
      console.warn(`Skipping invalid ${type} buffer at index ${i}`, typeof buffers[i], buffers[i]?.constructor?.name);
      continue;
    }
    
    const mimetype = mimetypes[i] || (type === 'audio' ? 'audio/ogg' : 'image/png');
    const originalname = filenames[i] || `${type}_${i + 1}.${type === 'audio' ? 'ogg' : 'png'}`;
...
export const processReceivedMessage = (req, res) => {
  try {
    ...
    const normalizedConversationId = conversation_id ? conversation_id.trim() : conversation_id;
    
    // Verificar se é uma nova conversa (apenas para USER e BOT, não ESCALATION)
    let finalClientName = client_name || null;
    let finalPhoneNumber = null;
    
    if (normalizedConversationId && flag !== 'ESCALATION' && isNewConversation(normalizedConversationId)) {
      ...
      console.log(`[NEW CONVERSATION] Assigned client: ${name} - ${phone} for conversation: ${normalizedConversationId}`);
    }
    ...
    let audioFiles = files.filter(f => f.fieldname.startsWith('audio_')).map(f => ({
      filename: f.filename,
      path: f.path,
      originalname: f.originalname,
      mimetype: f.mimetype,
    ...
    let imageFiles = files.filter(f => f.fieldname.startsWith('image_')).map(f => ({
      filename: f.filename,
      path: f.path,
      originalname: f.originalname,
      mimetype: f.mimetype,
    ...
    if (Array.isArray(images) && images.length > 0) {
      const processedImages = processBuffers(images, imageMimetypes, imageFilenames, 'image', uploadDir);
      imageFiles = [...imageFiles, ...processedImages];
    }
    
    if (Array.isArray(audios) && audios.length > 0) {
      const processedAudios = processBuffers(audios, audioMimetypes, audioFilenames, 'audio', uploadDir);
    ...
    if (imageUrl && typeof imageUrl === 'string') {
      ...
      const urlExtension = urlFilename.includes('.') ? urlFilename.split('.').pop() : 'jpg';
      imageFiles.push({
        url: imageUrl,
      ...
      console.log('[processReceivedMessage] Added external image URL:', imageUrl);
    }
    ...
    try {
      ...
      if (finalClientName && finalPhoneNumber) {
        updateConversationClientData(normalizedConversationId, finalClientName, finalPhoneNumber);
        console.log(`[NEW CONVERSATION] Updated all messages in conversation ${normalizedConversationId} with client data`);
      }
      
      // Se há escalação, atualizar dialog_info no widget via WebSocket
      if (hasEscalation(normalizedConversationId)) {
        ...
        console.log(`[processReceivedMessage] Updated dialog_info via WebSocket for conversation ${normalizedConversationId}`);
      }
    } catch (error) {
...
export const processSentMessage = async (req, res) => {
  try {
    ...
    const { flag, timestamp, message_id, conversation_id, user_id, text, client_name } = req.body;
    ...
    if (text && text.trim()) {
      console.log('[BRIDGE] Initiating bridge message send for message:', messageId);
