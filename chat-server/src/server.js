...
try {
  ...
  if (cleanupResult.updated > 0) {
    console.log(`[Migration] Cleaned up "gotcha" references: ${cleanupResult.updated} records updated out of ${cleanupResult.total} total`);
  }
} catch (error) {
  console.warn('[Migration] Error cleaning up gotcha references (non-critical):', error.message);
}
