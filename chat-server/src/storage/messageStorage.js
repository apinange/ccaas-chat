...
export const updateConversationClientData = (conversationId, clientName, phoneNumber) => {
  const normalizedId = conversationId ? conversationId.trim() : conversationId;
  const stmt = db.prepare(`
    UPDATE messages 
...
export const getUserMessagesSinceLastBeforeEscalation = (conversationId) => {
  ...
  const escalationIndex = allMessages.findIndex(msg => msg.flag === 'ESCALATION');
  ...
  const lastBeforeEscalation = messagesBeforeEscalation[messagesBeforeEscalation.length - 1];
  const lastBeforeEscalationTime = new Date(lastBeforeEscalation.created_at).getTime();
